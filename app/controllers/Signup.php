...
class Signup extends CI_Controller{
   ...
   public function profile( ){
     ...
     $data['docphoto']    = isJSON($researcher->docphoto) ? json_decode($researcher->docphoto , true) : [];
     $data['docidpass']   = isJSON($researcher->docidpass) ? json_decode($researcher->docidpass , true) : [];
   ...
   public function upload(){
    ...
    if(isset($_FILES)){
        ...
        $expected_files                         =  [];
        $expected_files['docphoto']             = 'Passport Photo';
  ...
  public function saveProfile( ){
       ...
       $researcher     = $this->researcher->get_researcher_by_email( $email   );
       $docphoto       = isJSON($researcher->docphoto) ? json_decode($researcher->docphoto , true) : [];
       $docidpass      = isJSON($researcher->docidpass) ? json_decode($researcher->docidpass , true) : [];
       ...
       if(!isset($docphoto['file_name']))
       {
           echo json_encode( [ 'success' => 0, 'message' => "Upload {$expected_files['docphoto']}" ] );
           die;
       }elseif(!isset($docidpass['file_name']))
       {
           echo json_encode( [ 'success' => 0, 'message' => "Upload {$expected_files['docidpass']}" ] );
           die;
       }

        $file_name   = $docphoto['file_name'];
        $folderid    = sha1($email);
        $upload_dir  = "uploads/appdocs/{$folderid}/";
